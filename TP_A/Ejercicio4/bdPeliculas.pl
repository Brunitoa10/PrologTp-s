...
% Part 1: Queries to answer the questions
...
% b. released_before(M, Y) <- the movie was released before the given year.
released_before(M, Y) :- movie(M, Year), Year < Y.

% c. same_year(M1, M2) <- the movies are released in the same year.
same_year(M1, M2) :- movie(M1, Y), movie(M2, Y), M1 \= M2.

% d. co_star(A1, A2) <- the actor/actress are in the same movie.
co_star(A1, A2) :- actor(M, A1, _), actor(M, A2, _), A1 \= A2.
co_star(A1, A2) :- actress(M, A1, _), actress(M, A2, _), A1 \= A2.
co_star(A1, A2) :- actor(M, A1, _), actress(M, A2, _).
co_star(A1, A2) :- actress(M, A1, _), actor(M, A2, _).